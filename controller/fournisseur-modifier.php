...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $reponse1 = $bdd->query("SELECT * FROM  fournisseur WHERE `id` = $id");
}
...
if(isset($_POST['update']))
{
    ...
    $req = $bdd->prepare('UPDATE fournisseur SET nom=:n,  telephone=:nm, adresse=:a, ville=:v , email=:e WHERE id=:i ');
    $req->execute(array(
        'n' => $nom,
        'nm' => $num,
        'a' => $adresse,
    ...
    header('Location: ../views/fournisseur.php?g=2');
}

if(isset($_POST['delete']))
{
    $id = $_POST['id'];
    $req1 = $bdd->prepare('DELETE FROM fournisseur WHERE id=:i ');
    $req1->execute(array(
        'i' => $id
    ));
    header('Location: ../views/fournisseur.php?g=3');
}
                        ...
                        <table class="taaaable table-bordered" id="table_abc">
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><input type="text" value="<?php echo $r['telephone'] ?>" name="num"> </td>
                                    <td><input type="text" value="<?php echo $r['adresse'] ?>" name="adresse"> </td>
                                    <td><input type="text" value="<?php echo $r['ville'] ?>" name="ville"> </td>
                                    <td><input type="email" value="<?php echo $r['email'] ?>" name="email"> </td>
                                </tr>
                            <?php endforeach;  ?>
                        </table>
                    <?php } ?>
